...
#NUMBA christofel coeficient calculation

@njit
def christoffel_schwarzschild_nb(x, rs):
    t, r, theta, phi = x
    f = 1-rs/r
    Gam = np.zeros((4,4,4), dtype=float) #CALCULATED BY HAND
    Gam[1,0,0] = 0.5*(f)*(rs/r**2)
    Gam[0,0,1] = 0.5*(rs/r**2)/(f)
    Gam[0,1,0] = Gam[0,0,1]
    Gam[1,1,1] = -0.5*(rs/r**2)/(f)
...
def trace_ray_fast_nb(x0, p0, r_far, max_steps, h_base, rs):
    ...
    for _ in range(max_steps):
        ...
        #adapt step size for larger curvature
        if r < 6.0:
            h *= 0.25
        if r < 4.0:
            h *= 0.1
...
#store data from traced ray in fast regime
@njit
...
#event horizon event checker for integrator
...
#define event for escaped ray for integrator
def event_far(lam, y, r_far):
    r = y[1]
    return r - r_far


#More robust ray tracer rk45 adaptive step for more complex ray paths
...
#decides which ray tracer to use
...
#photon momentum from camera
def photon_momentum_from_angles(u_cam, e_r, e_theta, e_phi, alpha_deg, beta_deg=0.0):
    alpha = np.deg2rad(alpha_deg)
    beta = np.deg2rad(beta_deg)

    # spatial unit direction in tetrad frame
    nhat = np.array([
        -np.cos(beta) * np.cos(alpha),   # -e_r
        np.sin(beta),                    # e_theta
        np.cos(beta) * np.sin(alpha)     # e_phi
    ])

    p = (u_cam * 1.0+ e_r*nhat[0]+e_theta*nhat[1]+e_phi*nhat[2])
    return p

#compute conserved Energy and L
...
#define ray direction from camera
@njit
def camera_ray_direction_nb(i, j, Nx, Ny, fov_x, fov_y, forward, right, up):
    ...
    # normalise in coefficient space
...
#render each indpendent row across cpu cores
def render_row(args):

    (j, Nx, Ny, fov, x0, u_cam, e_r, e_theta, e_phi, r_far, h_fast, max_steps_fast, b_crit, delta_b, col_hole, col_bg) = args
    ...
    for i in range(Nx):
        ...
        # hybrid tracer
        code, L = trace_ray_hybrid(x0, p0, r_far, h_fast=h_fast, max_steps_fast=max_steps_fast, lam_max_scipy=500.0, rtol=1e-6, atol=1e-6, max_step=0.5, b_crit=0,  delta_b=0)
        ...
        if code == 1:      # horizon
            ...
            color = np.array([0.0, 0.0, 1.0])  # debug
